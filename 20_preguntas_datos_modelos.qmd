...
-----
*Esta sección queda muy larga, por lo que podría introducir alguna segunda tabla con ejemplos de muestreo y problemas*
...
-   En los **datos de serie temporal** disponemos de múltimes observaciones de una unidad de investigación tomada secuencialmente en el tiempo, normalmente a intervalos regulares. Es decir, $t>>n$. El ejemplo paradigmático son datos sobre la evolución de variables macroeconómicas con el PIB, el IPC, o el precio de valores como el gas o el petróleo. 
...
---
*prompt para claude:*
estoy pensando en cual es el orden óptimo para presentar las clasificaciones de los modelos (a pesar de que entiendo que no son clasificaciones que siguen un orden jerárquico o secuencial -es decir, en forma de árbol - sino que son clasificaciones por criterios independientes, supongo que hay maneras más claras que otras de presentarlo.



Te comparto lo que tengo de momento. Ten en cuenta que estoy aún en un capitulo introductorio y que el objetivo de algún modo es redirigir el hilo al modelo de regresión, que desarrollaré en el siguiente capítulo. 
---

Como te puedías imaginar a estas alturas, empezemos por ver qué clases de modelos existen. 

En primer lugar, diferenciamos entre modelos estructurales y modelos de forma reducida:\[\^20-nota-7\] 

\[\^20-nota-7\]: He tradiucido directamente el concepto directamente del inglés, donde se usa *reduced form models*. Si alguien conoce una traducción mejor agradezcería que me la compartiera. 
...
La teoría económica nos dice que la cantidad demandada $Q^d$ depende del precio $P$ y de la renta de los consumidores $Y$; que la cantidad ofrecida $Q^s$ depende del precio $P$ y del coste de producción $W$, y que, en equilibrio, la cantidad ofrecida es igual a la demandada. 
Esto se traduce en un sistema de ecuaciones donde el precio y la cantidad se determina de forma simultánea.

$$
Q^d = \alpha_0 + \alpha_1 P + \alpha_2 Y + \varepsilon^d
$$
$$
Q^s = \beta_0 + \beta_1 P + \beta_2 W + \varepsilon^s
$$
$$
Q^d = Q^s
$$

Otro ejemplo completamente distinto es el modelo que usé en el Trabajo de Fin de Máster que realicé junto con Cristobal Blanco y Tomás Butelman. 
Para responder a nuestra pregunta de investigación, a saber, cuál es la probabilidad de que España cumpla su objetivo de producir 81% de electricidad de fuentes renovables en 2030, modelamos el funcionamiento del mercado eléctrico español de forma que el precio de la electricidad, la demanda y la cantidad generada por cada tecnología se resolvían simultáneamente.\[\^20-nota-8\]

\[\^20-nota-8\]: Podeis ver el código del proyecto este [repositorio de GitHub](https://github.com/PauOrive/Spain_ren_gen_2030)

No obstante, en econometría predominan los modelosde forma reducida, normalmente definidos mediante modelos de regresión. 
*quizás aquí introducir la notación y = f(X)?, porque conecta con la la distinción entre paramétrico y no paramétrico... o no?*


En segundo lugar, los modelos pueden ser paramétricos o no paramétricos dependiendo si asumen una forma funcional o no. 

-   Los **modelos paramétricos** especifican una forma funcional en los datos.

-   Los **modelos no paramétricos** dejan que los datos determinen la forma libremente (*tiene sentido esto? me parece poco claro desde un punto de vista pedagógico... quizás habría que hablar del trade-off*)

Otra clasificación más habitual en la jerga del *machine learning* es la distinción entre modelos supervisados y no supervisados. La diferencia radica en si hay o no una variable de interés. 

-   Los **modelos supervisados** contienen una variable de interés, normalmente referida como $y$, a la que queremos aproximarnos.

-   En los **modelos no supervisados** no hay ningñúna variable de interés, sino que buscan patrones y estructuras que en los datos que son ocultos a simple vista. 


Podemos clasificar casi cualquier modelo bajo estas tres categorías.
No obstante, la realidad tampoco se corresponde con una matriz de 3x3 donde cada opción es igual de plausible. 
Por ejemplo, (*explicar casos comunes y casos muy poco probables*)
...
Podemos representar esto de forma general como $y=f(X)$, donde $f$ es la función que relaciona ambas; su forma concreta dependerá del modelo que especifiquemos.
