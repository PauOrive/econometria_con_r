...
prompt a Claude: ver archivo 20_prompt_claude.qmd
...
En la práctica, los economistas suelen trabajar con datos administrativos, registros públicos o encuestas que ya se han llevado a cabo por organismos estadísticos, que son los que se encargan de diseñar encuestas y técnicas de muestreo.\[\^20-nota-3\] 
Sin embargo, es importante conocer ciertos conceptos clave y revisar la metodología de obtención de la muestra y, en consecuencia, ser honesto sobre a qué población podremos generalizar las conclusiones.
...
Por ejemplo, si queremos estudiar características de personas, pero sólo disponemos de un listado de viviendas, de modo que el muestreo aleatorio se realiza sobre viviendas.
...
Esta estrategia se suele usar cuando queremos asegurarnos de que todos los valores que toman ciertas variables estan representados (por ejemplo, que haya personas de todas las comunidades autónomas).\[\^20-nota-4\]

\[\^20-nota-4\]: En cualquier caso, si la muestra es suficientemente grande, el muestreo aleatorio simple es equivalente al estratificado, puesto que la distribución de una muestra aleatoria se asemejará cada vez más a la distribución de la población a medida que crece el tamaño muestral.

Cuando el muestreo no se realiza de forma aleatoria, corremos el riesgo de introducir *sesgo de selección*.
Los casos más frecuentes son cuando los datos se recogen sobre las personas que se presenten voluntarias, ya 
solo aquellos con opiniones extremas o alto interés responden, o sobre las personas que nos son más fácil de contactar (por ejemplo, los primeros que nos encontremos por la calle, ya que directamente exluye a todas las personas que no salen a la calle). 
Este es el caso de bastantes Trabajos de Fin de Grado, que suelen basarse en encuestas que han difundido a sus contactos por WhatsApp. 
Lo más probable es que las contesten personas que sientan el compromiso de hacerlo (familiares, amigos cercanos o compañeros de clase) o personas que estén muy motivadas a responder porque les interesa mucho la temática, lo que probablemente no sea representativo de la población objetivo del estudio.

Por último, pero no menos importante, debemos prestar atención al tamaño de la muestra. En términos generales, a mayor tamaño, mejor.
Como veremos en los siguientes capítulos, los requerimientos de tamaño muestral para que la inferencia sea válida dependen de la técnica que usemos (por ejemplo, un modelo de variables instrumentales requiere más observaciones que una regresión simple).
