...
En este libro me centro casi exclusivamente en los datos tabulares, es decir, aquellos que están organizados (o pueden organizarse) en filas (observaciones) y columnas (variables). 
...
Los datos pueden clasificarse, por lo menos según dos criterios.
...
-   En los **datos de sección cruzada** disponemos de una observación para cada unidad de investigación tomada en un momento determinado. Es decir, $t=1$ y $n>>t$.\[\^20-nota-5\]    
...
\[\^20-nota-5\]: Uso la notación con doble ">>" como en $n>>t$ para indicar que "$n$ es *mucho* más grande que $t$". Véase la @sec-convenciones-notacion para consultar todas las convenciones que uso en cuanto a la notación. 

Es importante no confundir los datos de panel con las secciones cruzadas repetidas, como la Encuesta de Población Activa (EPA), que entrevista cada trimestre a una muestra distinta de hogares. 
En ambos casos tenemos observaciones en varios momentos del tiempo, pero en la EPA no seguimos a los mismos individuos, sino que cada ola extrae una muestra nueva. 
Un panel longitudinal, en cambio, sigue a las mismas unidades a lo largo del tiempo.
...
Tal y como he explicado en la @sec-cual-es-tu-objetivo, el tipo de pregunta de investigación determina el curso de acción. Sin embargo, una vez tenemos definidos los datos 



El remanente de este capítulo y del resto de esta parte del libro está enfocado a (el caso de la inferencia causal)?
